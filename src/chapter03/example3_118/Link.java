...
public class Link {
    ...
    private class Node {
        ...
        private boolean containsNode(String data) {
            if (this.data.equals(data)) {                       //当前节点数据为要查询的数据
                return true;                                    //后面不再查询
            } else {                                            //当前节点数据不满足查询条件
         ...
         * @param data  新数据
         */
        private void setNode(int index, String data) {
            //使用当前的foot内容与要查询的索引进行比较，随后将foot的内容递增，目的是下次查询方便
         ...
         * @param data     要删除的数据
         */
        private void removeNode(Node previous, String data) {
    ...
    public void remove(String data) {
        if (this.contains(data)) {                          //主要功能是判断数据是否存在
            ...
            if (this.root.data.equals(data)) {              //根节点数据为要删除数据
    ...
    public String[] toArray() {
        ...
        this.foot = 0;                              //脚标清零操作
        this.retArray = new String[this.count];     //根据保存内容开辟数据
